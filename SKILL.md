---
name: zscc配图生成器
description: zscc配图生成器；当用户提到封面、正文配图、配图、知识图解、内容拆图、批量生图、文章配图、手绘知识卡片、小红书、公众号配图，或说“生成一个 logo / 图标 / 小图标 / app icon”，或指定内置 49 套内容风格与 8 套 logo/图标风格时使用。将中文主题、文章、段落、知识点或产品概念拆解为多风格封面、正文图、logo/图标与批量生图方案；可通过 visualize 交互选择推荐风格、封面比例尺寸和正文配图数量，并可在可用 image_gen 时隐藏提示词直接批量生图。
---
...
8. 若用户未指定完整出图配置且当前环境可用 `visualize`，先按“可视化交互选型”生成一次选择器；不要用连续文字追问替代。若不可用 `visualize`，普通内容默认使用手绘知识风和本节默认数量。
9. 若用户说“封面用 A，正文用 B”，封面和正文分别套用对应 style_id。
10. 若用户说“用合适的风格”“帮我选风格”“随机风格”，不要真正随机；根据内容主题、使用场景和任务类型按自动匹配规则选择最合适的 style_id。
11. 用户已明确 style_id、封面比例和正文图数量，或明确说“直接生成 / 用默认值 / 不用选择”时，跳过选择器并直接执行。
...
当 `visualize` 可用且用户尚未给出完整出图配置时，在调用 `image_gen` 前执行一次可视化选择。完整交互规范见 `references/interactive_selection.md`。

1. 先理解主题或文章，再按自动匹配规则计算 3 个推荐风格；按匹配度排序并置顶，第一项默认选中。推荐结果必须来自当前 49 套内容风格或 8 套图标风格，不能临时发明 style_id。
2. 使用 `visualize` 创建一张精炼的磨砂编辑感交互界面。推荐区优先显示已有示例图、中文风格名和一句推荐理由；style_id 只保存在控件数据与提交协议中，禁止出现在任何用户可见位置。
...
8. 若 `visualize` 不可用、界面提交失败，或当前客户端不支持交互，直接使用已计算的第一推荐风格、推荐比例和推荐数量继续，不要卡住任务。
9. 视觉层级、磨砂质感、中文文案、卡片排版与响应式细节必须遵守 `references/interactive_selection.md`；不要把风格名、英文 ID 和理由压进同一行，也不要让控件标签与数值错位。
