---
name: zscc配图生成器
description: 中文知识手绘配图生成器；当用户提到封面、封面图、正文配图、配图、知识图解、内容拆图、批量生图、批量配图、文章配图、把文章做成图、把文章拆成几张图、根据文章生成封面和正文图、手绘知识卡片、知识卡片风格、小红书或公众号配图、米白背景手绘浅色块等意图时使用。将中文主题、文章、段落或知识点拆解为统一暖白纸感、黑灰细线、低饱和浅色块、成熟中文手写字风格的封面、正文图与批量生图方案；支持在可用图片生成工具时隐藏提示词并直接批量生图。
---
...
把中文文章、选题、段落或知识点转成一套统一风格的知识配图：封面图、正文配图、内容拆图和批量生图清单。输出要像高质量中文知识博主的手绘内容资产，而不是 PPT、课程课件、商业海报或廉价信息图。
...
6. 不要追问风格细节；按本 Skill 的固定风格补全缺失字段。

## 视觉锚点

每个最终图片 prompt 都必须包含这个全局风格锚点：
...
详细风格规范见 `references/visual_style.md`；封面与正文模板见 `references/cover_prompt.md`、`references/body_prompt.md`。
