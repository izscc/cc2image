...
一个用于 Codex 的中文文章视觉系统生成 Skill。

它不是单一“配图模板”，而是一个「文章理解 + 内容拆图 + 风格匹配 + 稳定生图提示词」工具：可以把中文文章、选题、段落或知识点，自动拆解成封面图、正文配图、系列主视觉和批量生图清单，并在支持图片生成工具的环境中直接批量生图。

默认风格是「手绘知识风」，同时内置 37 套可选视觉风格，覆盖知识图解、东方人文、极简编辑、字体材质、微缩场景、品牌海报和社会议题封面等方向。用户不指定风格时默认使用 `handdrawn_knowledge_card`；用户说“合适的风格”“帮我选风格”“随机风格”时，会根据内容合理选择，不做纯随机。
...
> 多数封面型风格更适合头图 / 海报 / 系列主视觉；正文解释图仍建议默认使用手绘知识风。若用户要“安慰人、表达情绪、做治愈图”，优先使用治愈漫画风；若用户要字体材质类封面，可在亚克力字风、纸雕字体风、透明字境风、毛绒字体风、云朵字体风、泡沫字体风、金属奢华风、编辑线稿风、具象标注风中选择。
