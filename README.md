...
一个用于 Codex 的中文文章配图 Skill。

它可以把中文文章、选题、段落、知识点，自动拆解成图片生成方案，并在支持图片生成工具的环境中直接批量生图。

默认风格是「手绘知识风」，同时内置 35 套可选风格。用户不指定风格时默认使用 `handdrawn_knowledge_card`；用户说“合适的风格”“帮我选风格”“随机风格”时，会根据内容合理选择，不做纯随机。
...
| `editorial_object_annotation_card` | 具象物品标注编辑风 | AI方法论、设计思维、知识卡片、认知模型、信任验证、工作流原则 |
...
> 多数封面型风格更适合头图 / 海报 / 系列主视觉；正文解释图仍建议默认使用手绘知识风。若用户要“安慰人、表达情绪、做治愈图”，优先使用治愈漫画风；若用户要字体材质类封面，可在亚克力字风、纸雕字体风、透明字境风、毛绒字体风、云朵字体风、泡沫字体风、金属奢华风、编辑线稿风、具象物品标注编辑风中选择。
...
```text
用具象物品标注编辑风，做一张关于“别急着生成，先验证”的 AI 方法论卡片。
```
